exports.handler = async function(event, context) {
  const headers = {
    'Access-Control-Allow-Origin': '*',
    'Content-Type': 'application/json'
  };

  const strategy = event.queryStringParameters?.strategy || 'default';

  try {
    // Регистрируем новое устройство в Cloudflare WARP
    // Каждый запрос → новый уникальный ключ
    const installId = generateInstallId();
    
    const registerRes = await fetch('https://api.cloudflareclient.com/v0a2158/reg', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        install_id: installId,
        tos: new Date().toISOString().replace(/\.\d+Z$/, 'Z'),
        type: 'Android',
        key: generateKey()
      })
    });

    if (!registerRes.ok) {
      throw new Error(`Cloudflare API error: ${registerRes.status}`);
    }

    const device = await registerRes.json();
    
    // Получаем конфиг
    const configRes = await fetch('https://api.cloudflareclient.com/v0a2158/config', {
      headers: { 'Authorization': `Bearer ${device.token}` }
    });
    
    const warpConfig = await configRes.json();

    // Параметры для разных стратегий (версия 1.5)
    const params = {
      default: { jc: 120, jmin: 23, jmax: 911, s1: 0, s2: 0, h1: 1, h2: 2, h3: 3, h4: 4 },
      stealth: { jc: 100, jmin: 20, jmax: 800, s1: 0, s2: 0, h1: 1, h2: 2, h3: 3, h4: 4 },
      aggressive: { jc: 140, jmin: 30, jmax: 1000, s1: 0, s2: 0, h1: 2, h2: 4, h3: 6, h4: 8 }
    };

    const p = params[strategy] || params.default;

    // Пакет маскировки (I1) — обязателен для версии 1.5
    const I1_VALUE = '<b >';

    // Собираем полный конфиг в формате AmneziaWG 1.5
    // Используем реальные ключи от Cloudflare WARP (уникальные)
    const awgConfig = `[Interface]
PrivateKey = ${warpConfig.private_key}
Address = ${warpConfig.addresses[0]}, ${warpConfig.addresses[1] || '2606:4700:110:8386:38b2:9eb5:ca47:3f2e'}
DNS = 1.1.1.1, 2606:4700:4700::1111, 1.0.0.1, 2606:4700:4700::1001
MTU = 1280

# AmneziaWG 1.5 обфускация
Jc = ${p.jc}
Jmin = ${p.jmin}
Jmax = ${p.jmax}
S1 = ${p.s1}
S2 = ${p.s2}
H1 = ${p.h1}
H2 = ${p.h2}
H3 = ${p.h3}
H4 = ${p.h4}
I1 = ${I1_VALUE}

[Peer]
PublicKey = ${warpConfig.peers[0].public_key}
Endpoint = ${warpConfig.peers[0].endpoint.host}:${warpConfig.peers[0].endpoint.ports[0]}
AllowedIPs = 0.0.0.0/0, ::/0
PersistentKeepalive = 25`;

    return {
      statusCode: 200,
      headers,
      body: JSON.stringify({
        success: true,
        config: awgConfig,
        strategy: strategy,
        version: "1.5",
        expires: new Date(Date.now() + 30 * 24 * 60 * 60 * 1000).toISOString()
      })
    };

  } catch (error) {
    console.error('Error:', error);
    return {
      statusCode: 500,
      headers,
      body: JSON.stringify({
        success: false,
        error: error.message,
        hint: 'Попробуйте другой режим или повторите позже'
      })
    };
  }
};

function generateInstallId() {
  return 'xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx'.replace(/x/g, () =>
    Math.floor(Math.random() * 16).toString(16)
  );
}

function generateKey() {
  const key = new Uint8Array(32);
  crypto.getRandomValues(key);
  // WireGuard требует определённые биты в первом и последнем байте
  key[0] &= 248;
  key[31] &= 127;
  key[31] |= 64;
  return Buffer.from(key).toString('base64');
}
