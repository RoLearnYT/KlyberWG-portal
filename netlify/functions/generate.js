exports.handler = async function(event, context) {
  const headers = {
    'Access-Control-Allow-Origin': '*',
    'Content-Type': 'application/json'
  };

  const strategy = event.queryStringParameters?.strategy || 'default';

  // Возвращаем только параметры обфускации (без реальных ключей)
  // Ключи будут генерироваться на стороне клиента
  const params = {
    default: { jc: 120, jmin: 23, jmax: 911, s1: 0, s2: 0, h1: 1, h2: 2, h3: 3, h4: 4 },
    stealth: { jc: 100, jmin: 20, jmax: 800, s1: 0, s2: 0, h1: 1, h2: 2, h3: 3, h4: 4 },
    aggressive: { jc: 140, jmin: 30, jmax: 1000, s1: 0, s2: 0, h1: 2, h2: 4, h3: 6, h4: 8 }
  };

  const p = params[strategy] || params.default;

  const I1_VALUE = '<b >';

  // Стандартный эндпоинт WARP (можно заменить на свой)
  const ENDPOINT = "104.16.0.1:2408";   // <- самый стабильный сейчас
  const PEER_PUBLIC_KEY = "bmXOC+F1FxEMF9dyiK2H5/1SU8HnrHD8+9psGJ6j2E8=";

  return {
    statusCode: 200,
    headers,
    body: JSON.stringify({
      success: true,
      strategy: strategy,
      params: p,
      i1: I1_VALUE,
      endpoint: ENDPOINT,
      peerPublicKey: PEER_PUBLIC_KEY,
      version: "1.5"
    })
  };
};
